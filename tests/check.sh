...
echo "Structure"
fires  "copula avoidance"    3 '\b(serves? as|stands? as|functions? as|boasts?|offers?|remains?)\b' '-i'
...
count "after fixed copula"         "$BAFTER" 0 '\b(serves? as|stands? as|functions? as|boasts?|offers?|remains?)\b' '-i'
...
count "paper before: copula"       "$TBEFORE" 2 '\b(serves? as|remains?|offers?|provides? an? [\w\s]{0,24}?(solution|approach|framework|means|basis))\b' '-i'
...
count "paper after: copula fixed"  "$TAFTER"  0 '\b(serves? as|remains?|offers?)\b' '-i'
...
count "prose before: copula"         "$PBEFORE" 2 '\b(boasts|offers|serves as|maintains)\b' '-i'
...
count "prose after: copula fixed"    "$PAFTER"  0 '\b(boasts|offers|serves as|maintains)\b' '-i'
